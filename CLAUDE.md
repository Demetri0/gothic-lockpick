...
- All app code lives in `index.html`, split into `<script>` blocks with **semantic ids** — `solver-src`, `state`, `ui-utils`, `game-logic`, `render`, `codecs`, `url`, `config`, `solve-ui`, `keyboard`, `init`, `worker-src`, `worker-host`, `db-search`. Grep `id="<name>"` to pull a specific block before editing.
