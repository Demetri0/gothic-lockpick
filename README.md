...
### Групповая оптимизация (`bfsSolveGrouped`)

Кратчайших по нажатиям решений обычно много; чистый BFS возвращает первое попавшееся, которое «зигзагом» скачет между плашками. Кнопка РЕШЕНИЕ использует `bfsSolveGrouped` — трёхфазный точный алгоритм:

1. BFS от старта → `distS` до обнаружения цели на глубине `L`
...
Итог: то же минимальное число нажатий, но минимально возможный список шагов (напр. 41 нажатие: 30 групп у чистого BFS → 11 у группового). Для патологически широких коридоров (>200 тыс. состояний) — жадный fallback «предпочитай ту же плашку», по-прежнему минимальный по нажатиям. Рандомизатор остаётся на чистом `bfsSolve` — ему нужна только длина решения.
